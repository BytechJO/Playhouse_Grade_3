function buildMcqBody(aObj) {
  ...
  if (aObj != undefined && aObj != null) {
    ...
    for (var x = 0; x < numOfQuestions; x++) {
      ...
      htmlStmt += '<div class="picks_grp w-100 background_audio">';
      ...
      let charIndex = 0; // Global character index for the continuous wave
      ...
      for (var y = 0; y < options.length; y++) {
          ...
          // Create wave effect by using a global index for all characters
          var waveText = options[y].text.split('').map((char) => {
              var offset = Math.sin(charIndex * 0.2) * 30; // Calculate offset based on global charIndex
              charIndex++; // Increment global index for each character
              return `<span style="display: inline-block; transform: translateY(${offset}px);">${char}</span>`;
          }).join('');
          htmlStmt += waveText;
    ...
    // NEW: one blank line PER expected word, instead of a single input
htmlStmt += '<div class="fillin_lines_row d-flex flex-wrap justify-content-center">';
var numBlanks = (tpOb.numFillinBlanks != undefined) ? tpOb.numFillinBlanks : 1;
for (var fb = 0; fb < numBlanks; fb++) {
    htmlStmt += '<div class="fill-group mx-2">';
    htmlStmt += '<input class="py-1 fillin_word_input" type="text" style="color: black;">';
    htmlStmt += '</div>';
}
htmlStmt += '</div>'; // end - fillin_lines_row
      htmlStmt += '</div>';
