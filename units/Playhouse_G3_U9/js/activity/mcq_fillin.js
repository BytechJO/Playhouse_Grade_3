...
MCQ.prototype = {
  ...
  validate: function () {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      // NEW: multiple independent blank lines, checked by MEMBERSHIP in
      // fillinAnswers (not fixed position) - same fix pattern as the drag&drop
      // column-answers issue: an empty blank is never counted as wrong, but
      // EVERY required word must appear correctly in SOME blank by the end.
      var wordInputs = elsQue[i].querySelectorAll(".fillin_word_input");
      var filledCorrect = true;

      if (wordInputs.length > 0 && fDataObj.fillinAnswers != undefined) {
        var correctList = fDataObj.fillinAnswers.map(function (w) {
          return w.toLowerCase();
        });
        var usedWords = [];

        for (var fw = 0; fw < wordInputs.length; fw++) {
          var val = wordInputs[fw].value.trim().toLowerCase();
          wordInputs[fw].style.color = "black";
          ...
          var isCorrectWord =
            correctList.indexOf(val) > -1 && usedWords.indexOf(val) == -1;
          if (isCorrectWord) {
            usedWords.push(val);
            wordInputs[fw].style.color = "green";
          } else {
            wordInputs[fw].style.color = "red";
            filledCorrect = false;
          }
        }

        // every required word must have been placed correctly somewhere
        if (usedWords.length < correctList.length) {
          filledCorrect = false;
        }
      } else if (inputBoxes.length == 1 && fDataObj.fillinanswer != undefined) {
        // fallback: keep old single-input behaviour for any question that
        // still uses the old "fillinanswer" format
        var userFillAns = inputBoxes[0].value.toLowerCase();
        var corrFillAns = fDataObj.fillinanswer.toLowerCase();
        filledCorrect = userFillAns == corrFillAns;
      }
