function buildMcqBody(aObj) {	
	var htmlStmt = '';
	if(typeof aObj !=undefined && aObj !=null){		
       
		var numOfQuestions = (aObj.questions).length;
		var numberofCols = parseInt(aObj.numberofcolumns);
		var numOfQinCol = Math.round(numOfQuestions/numberofCols);
		var currQueNum = 0;				
		// NEW: turn on the "per-question side image" layout (like the playhouse
		// "never / sometimes / often / usually / always" page) by setting
		// aObj.showQuestionImages = true in the data file. Everything else
		// keeps working exactly as before when this flag is not set.
		var useSideImages = (aObj.showQuestionImages == true);
		
		htmlStmt +=  '<div class="sub_footer_icon subFooterNav backNav mx-1">'
		htmlStmt +=  '<a href="">'
		htmlStmt +=  '<img src="../images/icons/back_btn.png" />'
		htmlStmt +=  '</a>'
		htmlStmt +=  '</div>'
		htmlStmt +=  '<div class="sub_footer_icon subFooterNav nextNav mx-1">'
		htmlStmt +=  '<a href="">'
		htmlStmt +=  '<img src="../images/icons/next_btn.png" />'
		htmlStmt +=  '</a>'
		htmlStmt +=  '</div>'
	    // ===================================================================== heading =====================
		htmlStmt += '<div class="act_head_group justify-content-center">';
		htmlStmt += '<div class="audioIcon off contant " data-slideNum="' + 1 + '" data-audio="' + aObj.mainTitleAudio + '">';
			htmlStmt += '<div class="q-type-img-container">';
			htmlStmt += '<img class="mainTitle" src=' + aObj.mainTitle + '>';
			if (aObj.mainTitleIcon != undefined && aObj.mainTitleIcon != '') {
				htmlStmt += '<img class="mainTitleIcon" src=' + aObj.mainTitleIcon + ' style="right:' + aObj.mainTitleIconPos.right + ';">';
			}
			htmlStmt += '</div>';
		htmlStmt += '</div>';
 
		htmlStmt += '<div class="activityHeading">'
			htmlStmt += '<div class="audioIcon off contant audioQuestionTitle" data-slideNum="' + 1 + '" data-audio="' + aObj.subTitleAudio + '">';
			htmlStmt += "<div class='page_sub_title'>";
				htmlStmt += "<p> " + aObj.subTitleTextLeft + " </p>";
				for (var sicons = 0 ; sicons < aObj.subTitleIcons.length ; sicons++) {
					htmlStmt += "<img src='" + aObj.subTitleIcons[sicons] + "'/>";
				}
				htmlStmt += "<p style='color:orange'> " + aObj.subTitleTextRight + " </p>";
			htmlStmt += "</div>";
			htmlStmt += '</div>';
		htmlStmt += '</div>';
		htmlStmt += '</div>';
		// ===================================================================== all_cont =====================
		htmlStmt += '<div class="options cont_ht_sf mx-auto">';
		htmlStmt += '<div class="all_cont justify-content-start justify-content-sm-center">';
        htmlStmt += '<div class="group_elm d-flex flex-wrap justify-content-center align-items-center mb-70">';
        
       
		for(x= 0;x<numberofCols;x++){	
			// when using per-question side images, the column always stacks
			// top-to-bottom (like the reference layout) instead of wrapping
			var tickGroupClass = useSideImages ? 'tick_group mcq_stacked d-flex flex-column' : 'tick_group d-flex flex-wrap';
			htmlStmt += '<div class="'+tickGroupClass+'">';			
			for(y= 0;y<numOfQinCol;y++){
				currQueNum++;
				var tpOb = (aObj.questions)[currQueNum -1];
				if(typeof tpOb != undefined && tpOb != null ){
 
					// -------- NEW: per-question side image wrapper -------- //
					var qImage = '';
					var qSide = (currQueNum % 2 == 1) ? 'left' : 'right';
					if (useSideImages) {
						qImage = (tpOb.image != undefined && tpOb.image != '') ? tpOb.image : '';
						qSide = (tpOb.imageposition != undefined && tpOb.imageposition != '') ? tpOb.imageposition : qSide;
						htmlStmt += '<div class="mcq_row mcq_row_' + qSide + '">';
						if (qImage != '' && qSide == 'left') {
							htmlStmt += '<div class="mcq_side_image"><img src="' + qImage + '"/></div>';
						}
					}
 
					htmlStmt += '<div class="que d-flex flex-column" id="que_'+currQueNum+'" data-qno="'+currQueNum+'">';
					htmlStmt += '<div class="d-flex q_part">';
					// htmlStmt += '<div class="q_num_space">a. </div>';
					if(aObj.numbering != 'none'){
						htmlStmt += '<div class="q_num_space">';
						if(aObj.numbering == 'alphabet'){
							xx =(currQueNum == 1) ?aObj.numberstartfrom:nextChar(xx);					
						}else if (aObj.numbering == 'number'){
							xx = (currQueNum - 1 + parseInt(aObj.numberstartfrom));
						}
						htmlStmt += xx+'</div>';
					}
					
					
					
					console.log(x, y, currQueNum, tpOb); 					
                    htmlStmt += '<div class="question">'+tpOb.question+'</div>';
					
					htmlStmt += '</div>'; // - /q_part
					htmlStmt += '<div class="d-flex flex-wrap picks_grp">';
					
						if((tpOb.options).length > 0){
							for(var opt=0;opt<(tpOb.options).length;opt++){
								
								htmlStmt += '<div id="pick_'+(currQueNum)+'_'+(opt+1)+'" class="pick d-flex">'; 
								if(tpOb.numbering != 'none'){
									htmlStmt += '<div class="q_num_space">';
									if (tpOb.numbering == 'alphabet') {
										xx = (opt == 0) ? tpOb.numberstartfrom : nextChar(xx);
									} else if (tpOb.numbering == 'number') {
										xx = (opt + parseInt(tpOb.numberstartfrom));
									}
									htmlStmt += xx + '. </div>';
								}
								htmlStmt += '<div class="txt">'+((tpOb.options)[opt]).text+'</div> ';    
								// htmlStmt += '<div><img src="'+((tpOb.options)[opt]).image+'"/></div>';                               
								htmlStmt += '</div>';
							}
							
						}
						htmlStmt += '</div>';// - /picks_grp
						// htmlStmt += '<div class="img_space"><img src="'+tpOb.image+'"></img></div>';
 
					htmlStmt += '<div class="icon_wrap p-2">';
					htmlStmt += '<div class="tick"><img src="../images/icons/check_btn.png"></div>';
					htmlStmt += '<div class="cross"><img src="../images/icons/cross_btn.png"></div>';
                    htmlStmt += '</div>';
                    
 
					
					htmlStmt += '</div>'; // -- /que
 
					if (useSideImages) {
						if (qImage != '' && qSide == 'right') {
							htmlStmt += '<div class="mcq_side_image"><img src="' + qImage + '"/></div>';
						}
						htmlStmt += '</div>'; // -- /mcq_row
					}
				}
			}									
			htmlStmt += '</div>';// --/ tick_group
							
		}
		if(aObj.rightImage != undefined && aObj.rightImage != ""){
			htmlStmt += '<div class="right-image-container">';
			htmlStmt += '<img class="right-image" src="' + aObj.rightImage + '" alt="image"/>';
			htmlStmt += '<div class="textOnImage-container">';
			for (t = 0; t < aObj.rightText.length; t++) {
				htmlStmt += ' <div class="audioIcon off textOnImage" data-audio="' + aObj.rightTextAudio[t] + '" style="display: block;">' + aObj.rightText[t] + '</div> ';
			}
			htmlStmt += '</div>';
			htmlStmt += '</div>';
		}
		// htmlStmt += '<div class="image-container">';
		// htmlStmt += '<img src="../images/pages/activities/p14.png"/>';
		// htmlStmt += '</div>';
		htmlStmt += '</div></div></div>'; // --/ d-flex flex-wrap / all_cont / options
		
	}
	console.log('htmlStmt >> fillin Built');
	$( ".activity_area" ).append( htmlStmt );	
	
	setLoadedStatus(getCurrFileOrDirectory('file'));
}
function nextChar(c) {
	return String.fromCharCode(c.charCodeAt(0) + 1);
}
 