function buildFillInBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		// ===========================================================
		htmlStmt += '<div class="screen_elements d-flex flex-wrap h-100">';
		if(aObj.image != undefined && aObj.image != ""){
			htmlStmt += '<div class="q_image"><img src="'+aObj.image+'"></img></div>';
		}
		htmlStmt += '<div class="group_elm d-flex flex-column">';
			...
			for (x = 0; x < numOfQuestions; x++) {
				...
				htmlStmt += '<div class="que img_fillin_gr d-flex flex-column" data-qno="' + (x + 1) + '" status="' + tmpObj.status + '">';
			...
			htmlStmt += '<div class="q_text">';
			if(aObj.text != undefined && aObj.text != ""){
				for(let i=0; i<aObj.text.length; i++){
					htmlStmt += '<div class="text">'+aObj.text[i]+'</div>';
				}
			}
			htmlStmt += '</div>';
		htmlStmt += '</div>';
