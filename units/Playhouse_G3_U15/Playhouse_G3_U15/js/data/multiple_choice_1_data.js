var stereo_data = {
  audio: "",
  exist: true,
  bgColor_rgb: "rgb(53, 130, 180)",
  type: "text",
  playListData: [
    {
      audiourl: "../audios/page_74/1.mp3",
    },
    {
      url: "",
    },
    {
      url: "",
    },
    {
      url: "",
    },
  ],
};

var mcq_data = {
  layout: 1,
  numinrow: [[1]],
  mainTitle: "../images/pages/sb-icons/word_main_title.png",
  mainTitleIcon: "",
  mainTitleIconPos: { right: "-18px" },
  mainTitleAudio: "../audios/page_74/WORD_POWER.mp3",
  subTitleTextLeft:
    "<span class='blue_text'>1</span> Circle the correct Word Power word for each picture.",
  subTitleTextRight: "",
  subTitleIcons: [],
  subTitleAudio:
    "../audios/page_74/1_Circle_the_correct_Word_Power_word_for_each_picture.mp3",
  select: "single", // single (or) multiple
  shape: "roundrect", // circle (or) roundrect (or) rectangle (or) svg (or) cross (or) checkbox
  bgcolor: "none",
  numbering: "number",
  numberstartfrom: 1,
  numberofcolumns: 1,
  questions: [
    {
      question: "",
      image: "../images/pages/activities/1-img-1.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "go along",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "turn right",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "roundabout",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-2.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "on the corner",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "go along",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "go up",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-3.png",
      answer: [3],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "go down",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "on the corner",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "roundabout",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-4.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "between",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "roundabout",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "go up",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-5.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "turn left",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "turn right",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "across from",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-6.png",
      answer: [3],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "go along",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "on the corner",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "go down",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-7.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "turn left",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "on the left",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "on the right",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-8.png",
      answer: [3],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "between",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "turn right",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "on the right",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
  ],
};
