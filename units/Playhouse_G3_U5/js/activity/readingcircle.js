//  ****************************************** //
//  ReadingCircle - Version no: 1
//  Click a target word (some/any) to cycle: none -> circle -> underline -> none
//  Validate compares each interactive word's mark against its data-correct attribute
//  (already computed at build time from the targetWords map).
//  ****************************************** //
window.ReadingCircle = function(obj, dataObj){
    ob = obj[0].getElementsByClassName("options");
    console.log('ReadingCircle > ', $('.activity_area'));
    this.settings = {
        'activity_area' : ob[0],
...
ReadingCircle.prototype = {
    ...
    listen:function(ob){
        ...
        var wordWraps = e.querySelectorAll('.rc_wordWrap');

        for (var i = 0; i < wordWraps.length; i++) {
            wordWraps[i].addEventListener('click', function(){
                var wordEl = this.querySelector('.rc_word');
    ...
    validate:function(){
        ...
        for (var i = 0; i < wordWraps.length; i++) {
            ...
            if(userMark == correctType){
                wordWraps[i].classList.remove('rc_wrong');
                wordWraps[i].classList.add('rc_right');
            }else{
                wordWraps[i].classList.add('rc_wrong');
                wordWraps[i].classList.remove('rc_right');
                allCorrect = false;
            }
        }
