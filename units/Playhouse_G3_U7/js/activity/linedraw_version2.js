...
LineDraw.prototype = {
    ...
    setValuesForLine:function(lineOb){
        ...
        (lineOb).startX =((lineOb).startNode).position().left - (st_margin_left + halfWidth) ;
        ...
        (lineOb).endX =((lineOb).endNode).position().left - (ed_margin_left + halfWidth) -10 ;
    ...
    ($lines).append('<svg width="100%" height="100%" style="position: absolute">' +
                '<path id="'+obj.name+'" d="M'+ obj.startX +' '+ obj.startY +' C '+ (obj.startX + obj.endX) / 2 +' '+ (obj.startY) +
                ', '+ (obj.startX + obj.endX) / 2 +' '+ (obj.endY + 15) +', '+ obj.endX +' '+ (obj.endY + 15) +
                '" stroke="'+ob.lineColor+'" stroke-width="3" stroke-dasharray="5,5" fill="transparent" />' +
        '</svg>');
    },
    validate:function(){ 
        var ob = this.ob;
