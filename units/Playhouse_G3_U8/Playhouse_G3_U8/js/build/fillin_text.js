		...
		htmlStmt += '<div class="group_elm '+aObj.parentClassName+'">';
			...
			if(aObj.image != 'no' && aObj.image != ""){
				if(aObj.imageposition == 'front'){
					htmlStmt += '<div class=""><img src="'+aObj.image+'" class="q_img"></div>';
				}
			}
			...
			for (x = 0; x < numOfQuestions; x++) {
				...
				if (tmpObj.images != '' && tmpObj.images != undefined) {
					htmlStmt += '<div class="image_space"><img src="' + tmpObj.images[0] + '"></div>';
				}
				...
				if(tmpObj.textEnd != undefined && tmpObj.textEnd != ""){
					htmlStmt += '<div class="fillin_end d-flex flex-wrap">'; 
						htmlStmt += '<div class="audioIcon off contant" data-audio="'+tmpObj.textEndAudio+'">' ;
							htmlStmt += '<div class="text_End d-flex flex-wrap">'+tmpObj.textEnd+'</div>';
						htmlStmt += '</div>';
						if(tmpObj.images && tmpObj.images.length > 1){
							htmlStmt += '<div class="image_space"><img src="' + tmpObj.images[1] + '"></div>';
						}
			...
			htmlStmt += '</div>'; // end - ques
			if(aObj.image != ""){
				htmlStmt += '<div class="image-wrap"><img src="' + aObj.image + '"/></div>';
			}
		htmlStmt += '</div>';
