function initActivity(activity){
	...
	jQuery.each(activity.questions, function(key, question){
		...
		html += '<li><span class="question_text">'+question+'</span><input type="text"  value="" style="display: none;" />&nbsp;&nbsp;<a href="javascript:void(0)" onclick="getSelectedText(this)"><img src="../images/icons/new_pencil_underline.png" /></a></li>';
	});
	html += '</ul>';
	html += '</div>';
...
function getSelectedText(obj){
	...
	// var val = "";
	var _val =jQuery(obj).parent().find('input[type="text"]').val();
	if(_val!="")
	{
	jQuery(obj).parent().find('input[type="text"]').val(_val + "," +selected_text.trim());
	}
	else{
