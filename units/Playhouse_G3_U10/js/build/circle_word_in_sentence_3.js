				...
				if(typeof tpOb != undefined && tpOb != null ){
                    ...
                    for (var rr = 0; rr < (opts).length; rr++) {
						...
						if(tpOb.inputbox == "no"){
							...
							tmphtml = "<span class='pick_set flex-wrap' id='pick_set_"+currQueNum+"_"+(rr+1)+"'><input class='text_input_area ' type='text' readonly disabled>";
						}						
                        var tmpOpt = opts[rr];
                        for(var aa=0;aa<tmpOpt.length;aa++){
							if(aa == 0){
								tmphtml += 'a <span class="pick" id="pick_'+currQueNum+'_'+(rr+1)+'_'+(aa+1)+'" data-opttext="'+tmpOpt[aa]+'">'+tmpOpt[aa]+'</span>';
                            } else {
								tmphtml += 'b <span class="pick" id="pick_'+currQueNum+'_'+(rr+1)+'_'+(aa+1)+'" data-opttext="'+tmpOpt[aa]+'">'+tmpOpt[aa]+'</span>';
							}
                        }
						tmphtml += '</span>';
