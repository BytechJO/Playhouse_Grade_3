...
DragDrop.prototype = {
    ...
    listen: function (ob) {
        ...
        // (locked words are worked examples and must stay put)
        $area.find('.draggable_word').not('.locked').draggable({
            revert: 'invalid',
            revertDuration: 200,
            helper: 'clone',
            appendTo: 'body',
        ...
        // (locked example words can't be removed by the student)
    ...
    reset: function () {
        ...
        // (locked worked-example words stay exactly where they are)
        $area.find('.placed_word').not('.locked').remove();
        $area.find('.clue_word').not('.locked').removeClass('used_in_bank');
