var stereo_data = {
  audio: "",
  exist: true,
  bgColor_rgb: "rgb(53, 130, 180)",
  type: "text",
  playListData: [
    {
      audiourl: "../audios/page_64/1.mp3",
    },
    {
      url: "",
    },
    {
      url: "",
    },
    {
      url: "",
    },
  ],
};

var mcq_data = {
  layout: 1,
  numinrow: [[1]],
  mainTitle: "../images/pages/sb-icons/word_main_title.png",
  mainTitleIcon: "",
  mainTitleIconPos: { right: "-20px" },
  mainTitleAudio: "../audios/page_64/WORD_POWER.mp3",
  subTitleTextLeft:
    "<span class='blue_text'>1</span> Circle the correct Word Power word.",
  subTitleTextRight: "",
  subTitleIcons: [],
  subTitleAudio: "../audios/page_64/1_Circle_the_correct_Word_Power_word.mp3",
  rightImage: "",
  rightText: [],
  rightTextAudio: [],
  select: "single", // single (or) multiple
  shape: "roundrect", // circle (or) roundrect (or) rectangle (or) svg (or) cross (or) checkbox
  bgcolor: "none",
  numbering: "number",
  numberstartfrom: 1,
  numberofcolumns: 1,
  defaultAnswer: [-1],
  questions: [
    {
      question: "",
      image: "../images/pages/activities/1-img-1.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "raincoat",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "skirt",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-2.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "skirt",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "cardigan",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-3.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "ski cap",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "boots",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-4.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "woollen scarf",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "checkered",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-5.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "ski cap",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "heels",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-6.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "heels",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "belt",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-7.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "ring",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "blouse",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-8.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "leather",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "earrings",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-9.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "bracelet",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "wallet",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-10.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "belt",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "raincoat",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-11.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "plaid",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "ski cap",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/1-img-12.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      numbering: "none", // alphabet (or) number (or) none
      numberstartfrom: "a",
      defaultAnswer_pick: [0],
      options: [
        {
          text: "umbrella",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "sunglasses",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
  ],
};
