...
FillIn.prototype = {
  ...
  listen: function (ob) {
    ...
    // NEW: click a word inside the parentheses to circle it (radio-style,
    // only one word per question can be circled at a time)
    var circleOpts = e.querySelectorAll(".circleOpt");
    for (var c = 0; c < circleOpts.length; c++) {
      circleOpts[c].addEventListener("click", function () {
        var parentQue = this.closest(".que");
        var siblingOpts = parentQue.querySelectorAll(".circleOpt");
        for (var s = 0; s < siblingOpts.length; s++) {
          siblingOpts[s].classList.remove("circled");
        }
  ...
  validate: function () {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      // NEW: check the circled word too
      var circledEl = elsQue[i].querySelector(".circleOpt.circled");
      var circleCorrect =
        circledEl != null &&
        circledEl.dataset.word == fDataObj.correctCircleWord;
      if (!circleCorrect) {
        _wrong++;
      }
  ...
  reset: function () {
    ...
    var circleOpts = e.querySelectorAll('.circleOpt');
    for (var c = 0; c < circleOpts.length; c++) {
        circleOpts[c].classList.remove('circled');
    }
