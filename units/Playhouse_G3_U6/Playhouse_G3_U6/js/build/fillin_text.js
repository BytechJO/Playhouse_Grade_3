function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    tmpObj = Object.assign({}, tmpObj, { text: wrapCircleOptions(tmpObj.text) }); // NEW
      ...
      if (tmpObj.singleword) {
        var str = tmpObj.text;
        var qStr =
          '<div class="txt-audioIcon off d-flex contant min_w_fit_contant" data-audio="' +
          tmpObj.textaudios[0] +
          '">' +
...
function wrapCircleOptions(text) {
  return text.replace(/\(([^)\/]+)\/([^)]+)\)/g, function (match, w1, w2) {
    w1 = w1.trim();
    w2 = w2.trim();
    return (
      '(<span class="circleOpt" data-word="' +
      w1 +
      '">' +
      w1 +
      "</span> / " +
