...
WordSearch.prototype = {
  ...
  cellDown: function (cell, evt) {
    ...
    if (ob._startSelect && cell != undefined) {
      ...
      ob._rectOb.left =
        $("." + thisCell).position().left + ob._rectOb.offsetPose;
  ...
  cellMove: function (cell) {
    ...
    if (ob._startSelect && cellCls != undefined) {
      ...
      if (fCell != undefined) {
        ...
        var tmpWidth = Math.sqrt(
          (fCell.position().left + ob._rectOb.offsetPose - ob._rectOb.left) *
            (fCell.position().left + ob._rectOb.offsetPose - ob._rectOb.left) +
            (fCell.position().top + ob._rectOb.offsetPose - ob._rectOb.top) *
              (fCell.position().top + ob._rectOb.offsetPose - ob._rectOb.top),
        );
        ob._rectOb.width = tmpWidth + Math.round(0.8 * fCell.outerWidth());
        var ang = self.angleBetweenTwoPoints(
          ob._rectOb.endX,
          ob._rectOb.top,
          fCell.position().left + ob._rectOb.offsetPose,
          fCell.position().top + ob._rectOb.offsetPose,
          ob._rectOb.left,
          ob._rectOb.top,
        );
  ...
  drawRect: function (ob, axn, nam) {
    ...
    if (axn == "start") {
      ...
      rect.css({
        "transform-origin":
          obj.transformPerc + "px" + " " + (obj.transformPerc + "px") + " 0px",
      });
    }
  ...
  validate: function (ob) {
    ...
    if (ob.allCorrect != 1) {
      ...
      selectAng = Math.round(selectAng / 45) * 45; // تقريب لأقرب 0/45/90/135/180 لتفادي فروقات البكسل عند السحب القطري
